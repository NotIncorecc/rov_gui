...
export default function CameraMonitoringTerminal() {
  const [logs, setLogs] = useState<string[]>([]);
  const [command, setCommand] = useState('');
  const [isConnected, setIsConnected] = useState(false);
  const logsEndRef = useRef<HTMLDivElement>(null);
  const router = useRouter();
  const sessionName = 'camera-monitoring-terminal';
  ...
  return (
    <div className="min-h-screen bg-black text-blue-400 font-mono flex flex-col">
      <div className="bg-gray-900 p-4 border-b border-gray-700 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-white">Camera Monitoring Terminal</h1>
      ...
      <div className="bg-gray-900 p-4 border-t border-gray-700">
        <div className="flex items-center">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-blue-400 outline-none placeholder-gray-500"
            autoFocus
          />
          <button
            onClick={sendCommand}
